...
func (repo *repository) getEntityBuilder(target string) (content.Builder, error) {
	...
	if !ok {
		return nil, fmt.Errorf(content.ErrTypeNotRegistered.Error(), entityType)
	}

	return fn, nil
}

func (repo *repository) Types() map[string]content.Builder {
